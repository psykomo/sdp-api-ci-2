...
use App\Modules\Visit\Models\VisitModel;
...
class VisitService
{
    public function __construct(
        protected VisitModel $visits = new VisitModel(),
    ...
    public function list(int $perPage = 10, ?string $search = null): array
    {
        ...
        $items = $builder->orderBy('visited_at', 'DESC')->paginate($perPage, 'visits');
        $pager = $this->visits->pager;

        return [
            'items' => $items,
            'meta'  => [
                'page'      => $pager->getCurrentPage('visits'),
                'perPage'   => $perPage,
                'total'     => $pager->getTotal('visits'),
                'pageCount' => $pager->getPageCount('visits'),
            ],
        ];
    }
